...
def load() -> GameObject:
    if len(GameObject.get_group_by_tag("replay_screen")) > 0:
        return GameObject.get_game_object_by_tags("replay_screen")
    
    scene = create_game_object(tags="replay_screen", size=WindowSize.value)
    ...
    world_section = create_game_object(
        parent=scene, 
        tags="replay_screen:world_section", 
    ...
    techs_window = create_list_game_object(
        parent=scene, 
        tags="replay_screen:techs_window", 
        at=InGrid((12, 8), (0, 0), (8, 8)), 
        color=(0, 70, 150), 
        shape=Shape.RECT, 
        axis=(1, 1), 
    ...
    info_section = create_game_object(
        parent=scene, 
        tags="replay_screen:info_section", 
    ...
    money_label = create_label(
        parent=info_section, 
        tags="replay_screen:info_section:money_label", 
        text=f"Money: -", font=pg.font.SysFont("consolas", WindowSize.value.inty() // 40), 
    ...
    selector_section = create_game_object(
        parent=info_section, 
        tags="replay_screen:info_section:selector_section",
    ...
    selector_image_section = create_game_object(
        ...
        tags="replay_screen:info_section:selector_section:selector_image_section",
        at=InGrid((4, 5), (0, 0), (1, 1)), 
        surface_margin=Vector2d(7, 2), 
    )
    selector_info_section = create_game_object(
        parent=selector_section, 
        tags="replay_screen:info_section:selector_section:selector_info_section",
    ...
    end_turn_button = create_game_object(
        parent=info_section, 
        tags="replay_screen:info_section:end_turn_button", 
    ...
    end_turn_label = create_label(
        ...
        tags="replay_screen:info_section:end_turn_label", 
        text="End Turn", 
    ...
    techs_button = create_game_object(
        parent=info_section, 
        tags="replay_screen:info_section:techs_button", 
    ...
    world_size = replay.Replay.world_size_as_Vector2d()
    world = create_game_object(world_section, "replay_screen:world_section:world", size=world_size * WindowSize.get_block_size(), color=(80, 80, 80), shape=Shape.RECT)
    unit_layer = create_game_object(world, "replay_screen:world_section:world:unit_layer", size=world_size * WindowSize.get_block_size(), shape=Shape.RECT, layer=3)
    city_layer = create_game_object(world, "replay_screen:world_section:world:city_layer", size=world_size * WindowSize.get_block_size(), shape=Shape.RECT, layer=2)
    ui_lauer = create_game_object(world, "replay_screen:world_section:world:ui_layer", size=world_size * WindowSize.get_block_size(), shape=Shape.RECT, layer=4)
    ...
    def move_camera(g_obj: GameObject, keys: list[int], *_):
        world_obj = GameObject.get_game_object_by_tags("replay_screen:world_section")
...
def start():
    scene = GameObject.get_game_object_by_tags("replay_screen")
    tech_win = GameObject.get_game_object_by_tags("replay_screen:techs_window")

    scene.enable()
    tech_win.disable()

    replay.Replay.create_initial_objects()
