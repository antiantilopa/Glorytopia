...
from shared.loader import load_mains, load_complex_types_full, load_assets
...
if __name__ == "__main__":
    ...
    def start_game():
        if new_game:
            host.game = Game(Pos(7, 7), len(host.game_manager.players), 2)
            i = 0
            j = 0
            for player in host.game.players:
                if isinstance(player, Bot):
                    player.pdata.id = j
                    host.create_object(player.pdata)
                else:
                    pdata = GamePlayer.joined_players[i]
                    pdata.id = j
                    player.set_pdata(pdata)
                    i += 1
                j += 1
